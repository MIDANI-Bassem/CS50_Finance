...
def index():
    ...
    cash = cash_db[0]["cash"]
    return render_template("index.html", datbase = transaction_db, cash=cash)
...
def buy():
    ...
    if request.method == "GET":
        ...
        if stock == None:
            return apology("Symbol not found")
        if shares < 0:
            return apology("Please put a valide number of shares")
...
def add_cash():
    ...
    if not new_cash:
        return apology("Please put the amount")
    user_id= session["user_id"]
    user_cash_db = db.execute("SELECT cash FROM users WHERE id = :id", id=user_id)
    user_cash = user_cash_db[0]["cash"]
...
def register():
    ...
    if not confirmation:
        return apology("Please cofirm your password")
...
def sell():
    ...
    if request.method == "GET":
        ...
        db.execute("UPDATE users SET cash = ? WHERE id = ?", updt_cash, user_id)
        date = datetime.datetime.now()
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (?, ?, ?, ?, ?)", user_id, symbol, (-1) * shares, stock["price"], date)
        flash("SOLD !")
        return redirect("/")
